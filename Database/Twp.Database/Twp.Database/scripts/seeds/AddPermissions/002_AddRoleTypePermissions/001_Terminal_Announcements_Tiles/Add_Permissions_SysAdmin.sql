...
BEGIN TRY
	BEGIN TRANSACTION ADD_PERMISSIONS_ROLE_TYPE
		...
		DECLARE 
			@ROLE_SYSTEM_ADMIN UNIQUEIDENTIFIER;
		...
		---SET ROLE IDS
		SELECT @ROLE_SYSTEM_ADMIN = Id FROM RoleType WHERE Name = 'SysAdmin';
		...
		SELECT 
			...
			(
				...
				WHERE RTP.RoleTypeId = @ROLE_SYSTEM_ADMIN			
			)
		...
		BEGIN
			...
			SELECT PermissionId, @ROLE_SYSTEM_ADMIN, ValueToUse FROM @SYSTEM_ADMIN_PERMISSIONS_TO_ADD
		END
