...
BEGIN TRY
	BEGIN TRANSACTION ADD_PERMISSIONS_ROLE_TYPE
		...
		DECLARE 
			@LABOR_USER_ID UNIQUEIDENTIFIER;
		...
		---SET ROLE IDS
		SELECT @LABOR_USER_ID = Id FROM RoleType WHERE Name = 'LaborUser';
		...
		SELECT 
			...
			(
				...
				WHERE RTP.RoleTypeId = @LABOR_USER_ID			
			)
		...
		BEGIN
			...
			SELECT PermissionId, @LABOR_USER_ID, ValueToUse FROM @LABOR_USER_PERMISSIONS_TO_ADD
		END
