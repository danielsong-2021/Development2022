...
BEGIN TRY
	BEGIN TRANSACTION ADD_PERMISSIONS_ROLE_TYPE
		...
		DECLARE 
			@TERMINAL_ADMIN_ID UNIQUEIDENTIFIER;
		...
		---SET ROLE IDS
		SELECT @TERMINAL_ADMIN_ID = Id FROM RoleType WHERE Name = 'TerminalAdmin';
		...
		SELECT 
			...
			(
				...
				WHERE RTP.RoleTypeId = @TERMINAL_ADMIN_ID			
			)
		...
		BEGIN
			...
			SELECT PermissionId, @TERMINAL_ADMIN_ID, ValueToUse FROM @TERMINAL_ADMIN_PERMISSIONS_TO_ADD
		END
